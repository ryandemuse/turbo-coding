...
void turbo_decode_4(int length, double *x_n, double *p1_n, double *p2_n, double *L_h, int *X_h, int *permutation) {
    ...
    for(ni = 0; ni < DEC_ITER; ni++) {
        ...
        BCJR_4(0,Lc,N,Le2_ip,LLout1,x_n,p1_n);
        ...
        BCJR_4(0,Lc,N,Le1_p,LLout2,x_np,p2_n);
...
void BCJR_4(int term, double Lc, int length, double *Lap, double *LLout, double *x_n, double *p_n) {
    ...
    double total;
    double PR1, PR0;
    double gamma[length][M8][2];                  // branch transition probabilities
    double gamma_ex[length][M8][2];               // branch extrinsic transition probs
    double alpha[length+1][M8];                   // alpha state probabilities
    double beta[length+1][M8];                    // beta state probabilities
    
    
    // initialize the alphas
    alpha[0][0] = 1.0;
    for(m = 1; m < M8; m++) {alpha[0][m] = 0.0;}
    ...
    for(n = 0; n < length; n++) {            // Loop through trellis time steps
        for(m = 0; m < M8; m++) {        // Loop through states at each time step
            // Now proceed for each databit
            
            p0 = parity_4[m][0] ? 1.0 : -1.0;
            p1 = parity_4[m][1] ? 1.0 : -1.0;
    ...
    for(n = 1; n <= length; n++) {
        ...
        for(m = 0; m < M8; m++) {
            alpha[n][m] = alpha[n-1][prev_4[m][0]] * gamma[n-1][prev_4[m][0]][0] + alpha[n-1][prev_4[m][1]] * gamma[n-1][prev_4[m][1]][1];
            
            total += alpha[n][m];
        }
        
        for(m = 0; m < M8; m++) {alpha[n][m] /= total;}
    }
    ...
    if(term) {
        beta[length][0] = 1.0;
        for(m = 1; m < M8; m++) {beta[length][m] = 0.0;}
    }
    else {
        for(m = 0; m < M8; m++) {beta[length][m] = 1.0 / ((double) M);}
    }
    ...
    for(n = length - 1; n >= 0; n--) {
        ...
        for(m = 0; m < M8; m++) {
            ...
            beta[n][m] = beta[n+1][next_4[m][0]] * gamma[n][m][0] + beta[n+1][next_4[m][1]] * gamma[n][m][1];
            
            total += beta[n][m];
        }
        
        // normalize at each time step
        for(m = 0; m < M8; m++) {beta[n][m] /= total;}
    }
    ...
    for(n = 0; n < length; n++) {
        ...
        for(m = 0; m < M8; m++) {
            PR1 += alpha[n][m] * gamma[n][m][1] * beta[n+1][next_4[m][1]];
            PR0 += alpha[n][m] * gamma[n][m][0] * beta[n+1][next_4[m][0]];
